...
- **Whether the list itself is inside the snippet is your choice, and it starts off.** By default the snippet is short — a `<div class="publist-embed">` carrying the settings, the two small-print lines, and the `<script>` tag — and the script fills the list in when the page loads. Tick **Include the list itself in the snippet (recommended)** and the wizard bakes a `<section class="publist">` copy of the list as it stands now into the snippet: that copy is in your page's HTML, so search engines index it and visitors with JavaScript disabled still see it, and it is on screen from the first paint instead of waiting for the fetch. The cost is length — the baked list is most of the snippet, which is why the box is a recommendation rather than the default. The script replaces the baked copy with a freshly fetched list on load either way.
- **The credit and source lines sit outside the baked copy**, as direct children of the `<div>`, so a short snippet still carries them. Leaving the snapshot out never takes them with it; they have their own checkboxes and nothing else.
- **It fails safe.** If the script cannot load, or an upstream API is down, whatever is in the container stays on the page — so with the box ticked your readers see the baked list rather than nothing. Nothing is ever blanked out; without the baked copy there is simply nothing yet to fall back on.
